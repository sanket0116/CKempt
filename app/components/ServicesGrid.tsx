          ...
          <div className="w-full lg:w-64 flex-shrink-0 space-y-2 py-4">
            {validCategories.map((category, index) => (
              <button
                ...
                className={`w-full text-center px-6 py-3.5 rounded-full font-semibold text-sm transition-all duration-300 ${activeTab === index
              ...
              >
                {validCategories[activeTab]?.services.map((service, idx) => (
                  ...
                  >
                    <Link
                      key={service.slug}
                      href={`/services/category/${validCategories[activeTab].key}`}
                      className={`group relative bg-white rounded-3xl p-6 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-gray-200 hover:border-transparent flex flex-col h-full ${showCards
              ...
              <div className="sm:hidden flex justify-center items-center pt-2 pb-4">
                <div className="flex space-x-2">
                  {validCategories[activeTab]?.services.map((_, idx) => (
                    <div
                      key={idx}
                      className={`w-1.5 h-1.5 rounded-full ${idx === currentSlide ? 'bg-[#FBB900]' : 'bg-gray-300'
                        }`}
                    />
                  ))}
                </div>
                      ...
                      {[0, 1].map((dotIndex) => {
                        ...
                        return (
                          <button
                            ...
                            className={`h-2 w-6 rounded-full transition-all duration-300 md:hidden ${isActive
                                ? 'bg-[var(--primary)]'
                      ...
                      {[0, 1, 2, 3].map((dotIndex) => {
                        ...
                        return (
                          <button
                            ...
                            className={`h-2 w-2 rounded-full transition-all duration-300 hidden md:block ${isActive
